...
;(function($, undefined) {
	...
	function Modal(options) {
		...
		//add by huangzh
		//添加提交模块功能
		if(typeof this.options.onSubmit === 'function' && !this.$modal.find('#scojsModalSubmit').length){
			this.$modal.find('.modal-footer').prepend('<button class="btn btn-info" id="scojsModalSubmit">提交</button>');
		}
		var self = this;
	...
	$.extend(Modal.prototype, {
		show: function() {
			...
			if(typeof this.options.onSubmit === 'function'){
				this.$modal.find('#scojsModalSubmit').on('click',this.options.onSubmit);
			}
			...
			if (this.options.remote !== undefined && this.options.remote != '' && this.options.remote !== '#') {
				...
				this.$modal.find('.inner').html("<div style='text-align:center;color:'>" +
						"<div class='progress'> <div class='progress-bar progress-bar-striped active' role='progressbar' aria-valuenow='60' aria-valuemin='0' aria-valuemax='100' style='width: 100%;'>正在加载...</div></div>" +
						"</div>");
		...
		,close: function() {
			...
			//add by huangzh
			this.$modal.find('#scojsModalSubmit').off('click');
			if (this.options.cssclass !== undefined) {
