...
;(function($, undefined) {
	...
	$.extend(Valid.prototype, {
		...
		show: function(errors) {
			//update by huangzh -- 2015年8月3日
			var self = this
				/*update by huangzh -- 2015年8月3日
				 * 增加tooltip模式 */
				if($.type(self.options.model) === 'string' && self.options.model == 'tooltip'){
					$.each(errors, function(k, v) {
						var $input = self.$form.find('[name="' + k + '"]');
						if (self.options.wrapper !== null) {
							$input.closest(self.options.wrapper).addClass('error');
						}
						Mypopover('#query_form [name="' + k + '"]',v,'top',false);
						$input.focus(function(){
							$('#query_form [name="' + k + '"]').popover('destroy');
						});
					});
				}else{
					$.each(errors, function(k, v) {
		...
		messages: {
			not_empty: '这个字段是必需的'
			,min_length: '请输入至少:value字符值'
			,max_length: '请输入不超过:value字符值。'
			,regex: ''
			,email: '请输入一个有效的电子邮件地址'
			,url: '请输入一个有效的URL'
			,exact_length: '请输入准确:value字符值'
			,equals: ''
			,ip: ''
			,credit_card: '请输入一个有效的信用卡号码'
			,alpha: ''
			,alpha_numeric: ''
			,alpha_dash: ''
			,digit: '请只输入数字'
