    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
            	//add by huangzh -- 2015/07/17 11:05
            	//获取关联数据,处理
            	var real_value_hzh = item[field];
            	if(field.indexOf('.') != -1){
            		var _field_hzh = field.split('.');
            		var _field_obj_hzh = item[_field_hzh[0]];
            		var _field_obj_property_hzh = _field_hzh[1].toString();
            		real_value_hzh = _field_obj_hzh[_field_obj_property_hzh];
            	}
                var text = '',
                    value = real_value_hzh,
                    type = '',
                    cellStyle = {},
                    id_ = '',
